...
def test_find_all_dirs(setup_fakefs):
    dirs = fs.find_all_dirs("/test")
    actual_paths = [d.path for d in dirs]
    actual_names = [d.name for d in dirs]
    expected_info = [
        ("/test", "test"),
        ("/test/basic/small", "small"),
        ("/test/basic/big", "big"),
        ("/test/special", "special"),
        ("/test/special", "special"),
    ]
    for expected_path, expected_name in expected_info:
...
def test_find_common_root(setup_fakefs):  #
    ...
    dir_a = Directory.from_path("/a")
    dir_b = Directory.from_path("/b")
    ...
    for d in dirs:
        d.path = f"/extra-root{d.path}"  # Mock new parent to /test, /extra-root
    root = fs.find_common_root(dirs)
    assert root is not None
...
def test_sort_dirs_dfs():
    ...
    expected = [Directory.from_path(p) for p in expected]
    dirs = expected[::-1]
