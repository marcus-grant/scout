...
def mk_scan_repo(db: DBConnector) -> ScanRepo:
...
class TestScanRepo:
    ...
    def test_finish_sets_finished_and_count(self, db: DBConnector) -> None:
        ...
        a, b = (scans := mk_scan_repo(db)).start(), scans.start()
        scans.finish(a, 7)
